...
export class MenuScene extends Phaser.Scene {
  ...
  _drawBackground() {
    ...
    for (let i=0;i<6;i++)
      this.add.circle(80+i*140, GROUND_Y+6, Phaser.Math.Between(10,20), COLORS.lavaDark, 0.45).setDepth(3);
  }
  ...
  _drawUI() {
    ...
    const volBtn = makeIconButton(this, GAME_WIDTH - 30, 96, 38,
      ...
      () => {
        if (!audio) return;
        audio.cycleVolume();
        volBtn.txt.setText(audio.volumeLabel());
      });
  ...
  _buildHelpOverlay() {
    ...
    const panel = this.add.rectangle(cx, cy, pw, ph, 0x1a0003, 0.97)
      .setStrokeStyle(2, 0xff4400).setDepth(31);
    ...
    const { bg: cbg, txt: ctxt } = makeButton(this, cx, cy + 62, 150, 38, '✕  CLOSE',
      () => this._toggleHelp());
